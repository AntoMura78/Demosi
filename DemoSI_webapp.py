...
# %% import_path
import_fecondità = "ISTAT_fecondità.csv"
import_path = st.text_input(
    "Inserisci il percorso dove si trova il file ISTAT_fecondità.csv (verifica che non ci siano modifiche nei nomi dei campi e nel separatore):")
# Controllo se il percorso è stato inserito
if import_path:
    st.write(f"Il percorso inserito è: {import_path}\ISTAT_fecondità.csv")
else:
    st.warning("Inserisci un percorso valido per continuare.")
# %% creazione del dataframe ed eliminazione dei campi ridondanti
# Specifica 'utf-8' se ci sono caratteri speciali
if import_path:
    file_path = os.path.join(import_path, import_fecondità)
    if os.path.isfile(file_path):
        st.success(f"File trovato: {file_path}")
    else:
        st.error(f"File non trovato nella directory")
    if os.path.isfile(file_path):
        db_fecondità = pd.read_csv(file_path, sep='|', encoding='utf-8')
        ...
        # %%
        st.write("Specifica i parametri del modello")
