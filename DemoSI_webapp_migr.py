...
scelta = st.radio(
    "Scegli un'opzione:",
    options=["Movimenti verso l'estero", "Movimenti interni"]
)
# %% load file
if scelta == "Movimenti verso l'estero":
    ...
    if Input_file:
        try:
            db_cancellati = pd.read_csv(Input_file, sep=',', encoding='utf-8')
        ...
        Input_file_2 = st.file_uploader(
            'Carica il file sulla popolazione straniera per classe di età ISTAT_Pop_str.csv, verifica che non vi siano cambiamenti nel nome dei campi e nel separatore')
        if Input_file_2:
            try:
            ...
            Input_file_3 = st.file_uploader(
                'Carica il file sulla popolazione totale per classe di età ISTAT_Pop_tot.csv, verifica che non vi siano cambiamenti nel nome dei campi e nel separatore')
            if Input_file_3:
                try:
                    ...
                    st.error('File errato o formato file non supportato')
                if "db_pop_classe_final" not in st.session_state["dataframes"]:
                    # Lista per accumulare i record
                    records = []
                    # Raggruppa per le colonne chiave
                    ...
                    st.write(
                        "Ecco un estratto della popolazione italiana e straniera per classe di età")
    ...
    if "db_cancellati_tasso_forecast" in st.session_state["dataframes"]:
        ...
        if Input_file_4:
            try:
                ...
                db_iscritti['Unicode'] = db_iscritti['ITTER107_B'] + \
                    db_iscritti['ISO']+db_iscritti['Sesso']+db_iscritti['ETA1']
                st.write(
